...
void devidetree( ) {
   ...
   TString name = TString("all");
   //Get old file, old tree and set top branch address
   TFile *oldfile = new TFile(name+".root");
   TTree *oldtree = (TTree*)oldfile->Get("ttree");
 ...
 oldtree->SetBranchAddress("Jet_ncHadrons", &Jet_ncHadrons, &b_Jet_ncHadrons);
   ...
   TFile *newfile = new TFile(name+"_train.root","recreate");
   TTree *newtree = oldtree->CloneTree(0);
   TFile *newfile2 = new TFile(name+"_test.root","recreate");
   TTree *newtree2 = oldtree->CloneTree(0);


   for (Long64_t i=0;i<nentries; i++) {
       oldtree->GetEntry(i);
       // remove gluon splitting, i.e. two b hadrons in a single jet
       /*  if (Jet_nbHadrons>1) {
	 //cout << "removed double b"<<endl;
	 continue;
	 
	 }*/
       if (i%10 == 0)     newtree2->Fill();
       else  newtree->Fill();
   }
