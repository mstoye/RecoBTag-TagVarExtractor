...
void PTEtaPlot( ) {
   ...
   TFile *oldfile = new TFile("QCDb.root");
   ...
   // TFile *newfile = new TFile(infile+"QCDb.root","recreate");
   // TTree *newtree = oldtree->CloneTree(0);
   // TH2D* thisshape = new TH2D("thisPtEta",";#eta;P_{T}", 6,0,2.4,20,30,300);

   TFile *refPTEtaFile = new TFile("all_PTETA.root","recreate");
   //   TH2D* refPTEta  = (TH2D*)refPTEtaFile->Get("bPtEta");
   TH2D* thisshape = new TH2D("PtEta",";#eta;P_{T}", 6,0,2.4,14,30,100);

   for (Long64_t i=0;i<nentries ; i++){
      oldtree->GetEntry(i);
      //     if(Jet_nbHadrons>1) continue;
      //     if(Jet_ncHadrons>1) continue;
